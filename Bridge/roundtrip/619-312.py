"""Round-trip check for Data/sheet-specs/619-312.json (Family 1).

Run: python Bridge/roundtrip/619-312.py
"""
...
spec = json.loads((ROOT / "Data/sheet-specs/619-312.json").read_text(encoding="utf-8"))
...
if squash('L/2') not in squash(all_text):
    fails.append('phrase missing: ' + 'L/2')
if squash('SHORT-TERM') not in squash(all_text):
    fails.append('phrase missing: ' + 'SHORT-TERM')
if squash('P, TMIA') not in squash(all_text):
    fails.append('PV phrase missing: ' + 'P, TMIA')
print('phrases ok')
# ---- sign size codes present in PDF text
need = ['G20-2', 'NYW8-33', 'R4-7', 'W4-2L', 'W9-3', 'W20-1', 'W20-5']
for code in need:
...
pdf_rows = extract_taper((90, 590, 400, 720), ytol=10, has_shoulder=False)
...
raw = group_rows(words_in_window(W, *(90, 150, 400, 280)), y_tol=8.0)
...
raw = group_rows(words_in_window(W, *(90, 40, 450, 150)), y_tol=5.0)
