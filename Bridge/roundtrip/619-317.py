"""Round-trip check for Data/sheet-specs/619-317.json (Family 1).

Run: python Bridge/roundtrip/619-317.py
"""
...
spec = json.loads((ROOT / "Data/sheet-specs/619-317.json").read_text(encoding="utf-8"))
...
# ---- phrase checks
if squash('SINGLE LANE') not in squash(all_text):
    fails.append('phrase missing: ' + 'SINGLE LANE')
...
if squash('MERGING') not in squash(all_text):
    fails.append('phrase missing: ' + 'MERGING')
if squash('P, TMIA') not in squash(all_text):
    fails.append('PV phrase missing: ' + 'P, TMIA')
print('phrases ok')
# ---- sign size codes present in PDF text
need = ['G20-2', 'NYW8-33', 'W4-2R', 'W20-1', 'W20-5']
for code in need:
...
pdf_rows = extract_taper((100, 160, 780, 350), ytol=8, has_shoulder=True)
...
raw = group_rows(words_in_window(W, *(90, 350, 450, 470)), y_tol=8.0)
...
raw = group_rows(words_in_window(W, *(100, 40, 500, 150)), y_tol=5.0)
