"""Round-trip check for Data/sheet-specs/619-523.json (Family 1).

Run: python Bridge/roundtrip/619-523.py
"""
...
spec = json.loads((ROOT / "Data/sheet-specs/619-523.json").read_text(encoding="utf-8"))
...
# ---- phrase checks
if squash('LONG TERM') not in squash(all_text):
    fails.append('phrase missing: ' + 'LONG TERM')
if squash('DOUBLE INTERIOR') not in squash(all_text):
    fails.append('phrase missing: ' + 'DOUBLE INTERIOR')
if squash('CHANNELIZING') not in squash(all_text):
    fails.append('phrase missing: ' + 'CHANNELIZING')
if squash('SEE NOTE 2') not in squash(all_text):
    fails.append('PV phrase missing: ' + 'SEE NOTE 2')
print('phrases ok')
# ---- sign size codes present in PDF text
need = ['G20-2', 'NYR9-11', 'NYW8-33', 'W4-2L', 'W20-1', 'W20-5']
for code in need:
...
pdf_rows = extract_taper((100, 143, 780, 330), ytol=10, has_shoulder=True)
...
raw = group_rows(words_in_window(W, *(90, 330, 450, 460)), y_tol=8.0)
